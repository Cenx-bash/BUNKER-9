...
function getMapType(level) {
  ...
  return MAP_TYPES[mapKeys[level % mapKeys.length]];
}
...
function updateAmmoDisplay() {
  ...
  if (isReloading) {
    ammoEl.textContent = "...";
    reloadEl.textContent = `RELOADING ${Math.ceil(reloadTimer * 10) / 10}s`;
...
function updateWeaponXPDisplay() {
  ...
  if (weapon.isMelee) {
    document.getElementById("weaponXpBar").style.width = "0%";
    document.getElementById("weaponXpText").textContent = "—";
    return;
  }
  const needed = getWeaponXpToLevel(weapon.level);
  const pct = Math.min((weapon.xp / needed) * 100, 100);
  document.getElementById("weaponXpBar").style.width = pct + "%";
  document.getElementById("weaponXpText").textContent =
    `${Math.floor(weapon.xp)}/${needed}`;
}
...
function updateStatsUI() {
  ...
  document.getElementById("statGrenades").textContent = stats.grenadesThrown;
  document.getElementById("statHordes").textContent = stats.hordesSummoned;
...
function togglePause() {
  ...
  document.getElementById("pauseMenu").style.display = gamePaused
    ? "flex"
    : "none";
...
function applyPerk(perkId) {
  ...
  updateStatsUI();
  showMessage(`✨ ${perk.name} equipped!`);
  return true;
}
...
function throwGrenade() {
  ...
  // 50% chance to spawn horde
  if (Math.random() < 0.5) {
    spawnHorde();
  }

  if (enemiesHit > 0) {
    showMessage(`💥 ${enemiesHit} enemies hit by grenade!`);
  }
}
...
function spawnHorde() {
  ...
  const hordeSize = Math.floor(10 + Math.random() * 40); // 10-50 enemies
  ...
  // Spawn horde enemies in a circle around the player
...
function createPolygonWeapon(weaponType) {
  ...
  switch (weaponType) {
    ...
    case "machete":
      ...
      const macheteHandleMesh = new THREE.Mesh(
        new THREE.CylinderGeometry(0.035, 0.045, 0.15, 6),
        macheteHandle,
      );
      macheteHandleMesh.position.set(0.2, -0.25, -0.75);
      macheteHandleMesh.rotation.x = Math.PI / 2;
      group.add(macheteHandleMesh);
      break;
    case "pistol":
      ...
      pistolBody.position.set(0.2, -0.18, -0.55);
      group.add(pistolBody);
      const pistolBarrel = new THREE.Mesh(
        new THREE.CylinderGeometry(0.025, 0.02, 0.25, 8),
        pistolMat,
      );
      pistolBarrel.position.set(0.2, -0.15, -0.85);
      pistolBarrel.rotation.x = Math.PI / 2;
      group.add(pistolBarrel);
      const pistolGripMesh = new THREE.Mesh(
        new THREE.BoxGeometry(0.04, 0.12, 0.05),
        pistolGrip,
      );
      pistolGripMesh.position.set(0.18, -0.34, -0.5);
      group.add(pistolGripMesh);
      const pistolSight = new THREE.Mesh(
        new THREE.BoxGeometry(0.01, 0.015, 0.015),
        pistolMat,
      );
      pistolSight.position.set(0.2, -0.1, -0.7);
      group.add(pistolSight);
      break;
    case "smg":
      ...
      smgMag.position.set(0.18, -0.32, -0.6);
      group.add(smgMag);
      const smgStockMesh = new THREE.Mesh(
        new THREE.BoxGeometry(0.06, 0.04, 0.08),
        smgStock,
      );
      smgStockMesh.position.set(0.2, -0.14, -0.3);
      group.add(smgStockMesh);
      const smgSight = new THREE.Mesh(
        new THREE.BoxGeometry(0.01, 0.02, 0.02),
        smgMat,
      );
      smgSight.position.set(0.2, -0.09, -0.75);
      group.add(smgSight);
      break;
    case "shotgun":
      ...
      shotBody.position.set(0.2, -0.18, -0.55);
      group.add(shotBody);
      const shotBarrel = new THREE.Mesh(
        new THREE.CylinderGeometry(0.035, 0.03, 0.4, 8),
        shotMat,
      );
      shotBarrel.position.set(0.2, -0.14, -0.95);
      shotBarrel.rotation.x = Math.PI / 2;
      group.add(shotBarrel);
      const shotGrip = new THREE.Mesh(
        new THREE.BoxGeometry(0.05, 0.14, 0.05),
        new THREE.MeshStandardMaterial({ color: 0x2a1a0a, roughness: 0.9 }),
      );
      shotGrip.position.set(0.18, -0.35, -0.5);
      group.add(shotGrip);
      const shotStock = new THREE.Mesh(
        new THREE.BoxGeometry(0.06, 0.03, 0.1),
        new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.8 }),
      );
      shotStock.position.set(0.2, -0.15, -0.3);
      group.add(shotStock);
      break;
    case "rifle":
      ...
      rifleBody.position.set(0.2, -0.18, -0.55);
      group.add(rifleBody);
      const rifleBarrel = new THREE.Mesh(
        new THREE.CylinderGeometry(0.025, 0.02, 0.4, 8),
        rifleMat,
      );
      rifleBarrel.position.set(0.2, -0.15, -1.0);
    ...
    case "energy":
      ...
      const energyBody = new THREE.Mesh(
        new THREE.BoxGeometry(0.06, 0.08, 0.4),
        energyMat,
      );
      energyBody.position.set(0.2, -0.18, -0.55);
      group.add(energyBody);
      const energyBarrel = new THREE.Mesh(
        new THREE.CylinderGeometry(0.02, 0.015, 0.3, 8),
        energyMat,
      );
      energyBarrel.position.set(0.2, -0.15, -0.9);
      energyBarrel.rotation.x = Math.PI / 2;
      group.add(energyBarrel);
      ...
      energyCore.position.set(0.2, -0.14, -0.55);
    ...
    default:
      ...
      defaultBody.position.set(0.2, -0.18, -0.55);
      group.add(defaultBody);
      const defaultBarrel = new THREE.Mesh(
        new THREE.CylinderGeometry(0.025, 0.02, 0.3, 8),
        defaultMat,
      );
      defaultBarrel.position.set(0.2, -0.15, -0.9);
      defaultBarrel.rotation.x = Math.PI / 2;
      group.add(defaultBarrel);
      const defaultGrip = new THREE.Mesh(
        new THREE.BoxGeometry(0.04, 0.12, 0.05),
        new THREE.MeshStandardMaterial({ color: 0x222222 }),
      );
      defaultGrip.position.set(0.18, -0.34, -0.5);
      group.add(defaultGrip);
  }
...
function createMapMonster(type, size, height, mapTheme) {
  ...
  switch (type) {
    case "grunt":
      ...
      armL.position.set(-size * 0.45, height * 0.35, 0);
      armL.rotation.z = 0.2;
      group.add(armL);
      const armR = new THREE.Mesh(armGeo, mat);
      armR.position.set(size * 0.45, height * 0.35, 0);
      armR.rotation.z = -0.2;
      group.add(armR);
      ...
      legL.position.set(-size * 0.2, height * 0.15, 0);
      group.add(legL);
      const legR = new THREE.Mesh(legGeo, mat);
      legR.position.set(size * 0.2, height * 0.15, 0);
      group.add(legR);
      break;
    case "fast":
      const slimMat = mat.clone();
      slimMat.color.setHex(0x224488);
      slimMat.emissive.setHex(0x4488ff);
      const slimBody = new THREE.Mesh(
        new THREE.CylinderGeometry(size * 0.25, size * 0.35, height * 0.5, 6),
        slimMat,
      );
      slimBody.position.y = height * 0.4;
      group.add(slimBody);
      const slimHead = new THREE.Mesh(
        new THREE.ConeGeometry(size * 0.25, height * 0.3, 6),
        slimMat,
      );
      slimHead.position.y = height * 0.85;
      group.add(slimHead);
      const slimArmGeo = new THREE.CylinderGeometry(
        size * 0.04,
        size * 0.05,
        height * 0.45,
        6,
      );
      const slimArmL = new THREE.Mesh(slimArmGeo, slimMat);
      slimArmL.position.set(-size * 0.35, height * 0.4, 0);
      slimArmL.rotation.z = 0.3;
      group.add(slimArmL);
      const slimArmR = new THREE.Mesh(slimArmGeo, slimMat);
      slimArmR.position.set(size * 0.35, height * 0.4, 0);
      slimArmR.rotation.z = -0.3;
      group.add(slimArmR);
      const slimLegGeo = new THREE.CylinderGeometry(
        size * 0.04,
        size * 0.04,
        height * 0.35,
        6,
      );
      const slimLegL = new THREE.Mesh(slimLegGeo, slimMat);
      slimLegL.position.set(-size * 0.2, height * 0.15, 0);
      group.add(slimLegL);
      const slimLegR = new THREE.Mesh(slimLegGeo, slimMat);
      slimLegR.position.set(size * 0.2, height * 0.15, 0);
      group.add(slimLegR);
      break;
    case "tank":
      const tankMat = mat.clone();
      tankMat.color.setHex(0x333311);
      tankMat.emissive.setHex(0x88aa44);
      const tankBody = new THREE.Mesh(
        new THREE.CylinderGeometry(size * 0.5, size * 0.6, height * 0.5, 8),
        tankMat,
      );
      tankBody.position.y = height * 0.4;
      group.add(tankBody);
      const tankHead = new THREE.Mesh(
        new THREE.BoxGeometry(size * 0.45, height * 0.25, size * 0.45),
        tankMat,
      );
      tankHead.position.y = height * 0.8;
      group.add(tankHead);
      const tankArmGeo = new THREE.CylinderGeometry(
        size * 0.1,
        size * 0.13,
        height * 0.3,
        6,
      );
      const tankArmL = new THREE.Mesh(tankArmGeo, tankMat);
      tankArmL.position.set(-size * 0.55, height * 0.35, 0);
      group.add(tankArmL);
      const tankArmR = new THREE.Mesh(tankArmGeo, tankMat);
      tankArmR.position.set(size * 0.55, height * 0.35, 0);
      group.add(tankArmR);
      const tankLegGeo = new THREE.CylinderGeometry(
        size * 0.1,
        size * 0.08,
        height * 0.25,
        6,
      );
      const tankLegL = new THREE.Mesh(tankLegGeo, tankMat);
      tankLegL.position.set(-size * 0.25, height * 0.15, 0);
      group.add(tankLegL);
      const tankLegR = new THREE.Mesh(tankLegGeo, tankMat);
      tankLegR.position.set(size * 0.25, height * 0.15, 0);
      group.add(tankLegR);
      break;
    case "brute":
      const bruteMat = mat.clone();
      bruteMat.color.setHex(0x441111);
      bruteMat.emissive.setHex(0xcc4444);
      const bruteBody = new THREE.Mesh(
        new THREE.CylinderGeometry(size * 0.45, size * 0.55, height * 0.5, 7),
        bruteMat,
      );
      bruteBody.position.y = height * 0.4;
      group.add(bruteBody);
      const bruteHead = new THREE.Mesh(
        new THREE.ConeGeometry(size * 0.35, height * 0.35, 5),
        bruteMat,
      );
      bruteHead.position.y = height * 0.85;
      group.add(bruteHead);
      const bruteArmGeo = new THREE.CylinderGeometry(
        size * 0.08,
        size * 0.12,
        height * 0.4,
        6,
      );
      const bruteArmL = new THREE.Mesh(bruteArmGeo, bruteMat);
      bruteArmL.position.set(-size * 0.5, height * 0.35, 0);
      bruteArmL.rotation.z = 0.15;
      group.add(bruteArmL);
      const bruteArmR = new THREE.Mesh(bruteArmGeo, bruteMat);
      bruteArmR.position.set(size * 0.5, height * 0.35, 0);
      bruteArmR.rotation.z = -0.15;
      group.add(bruteArmR);
      const bruteLegGeo = new THREE.CylinderGeometry(
        size * 0.08,
        size * 0.06,
        height * 0.3,
        6,
      );
      const bruteLegL = new THREE.Mesh(bruteLegGeo, bruteMat);
      bruteLegL.position.set(-size * 0.25, height * 0.15, 0);
      group.add(bruteLegL);
      const bruteLegR = new THREE.Mesh(bruteLegGeo, bruteMat);
      bruteLegR.position.set(size * 0.25, height * 0.15, 0);
      group.add(bruteLegR);
      const spikeGeo = new THREE.ConeGeometry(size * 0.06, height * 0.15, 4);
      const spikeL = new THREE.Mesh(spikeGeo, bruteMat);
      spikeL.position.set(-size * 0.45, height * 0.65, size * 0.15);
      spikeL.rotation.x = 0.3;
      group.add(spikeL);
      const spikeR = new THREE.Mesh(spikeGeo, bruteMat);
      spikeR.position.set(size * 0.45, height * 0.65, size * 0.15);
      spikeR.rotation.x = 0.3;
      group.add(spikeR);
      break;
    case "spitter":
      const spitMat = mat.clone();
      spitMat.color.setHex(0x113311);
      spitMat.emissive.setHex(0x44ff44);
      const spitBody = new THREE.Mesh(
        new THREE.CylinderGeometry(size * 0.35, size * 0.5, height * 0.4, 6),
        spitMat,
      );
      spitBody.position.y = height * 0.3;
      group.add(spitBody);
      const spitHead = new THREE.Mesh(
        new THREE.SphereGeometry(size * 0.35, 6, 6),
        spitMat,
      );
      spitHead.position.y = height * 0.7;
      spitHead.scale.set(1, 0.8, 1.2);
      group.add(spitHead);
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const tentacle = new THREE.Mesh(tentacleGeo, spitMat);
      ...
      const spitLegGeo = new THREE.CylinderGeometry(
        size * 0.04,
        size * 0.04,
        height * 0.2,
        5,
      );
      const spitLegL = new THREE.Mesh(spitLegGeo, spitMat);
      spitLegL.position.set(-size * 0.2, height * 0.1, 0);
      group.add(spitLegL);
      const spitLegR = new THREE.Mesh(spitLegGeo, spitMat);
      spitLegR.position.set(size * 0.2, height * 0.1, 0);
      group.add(spitLegR);
      break;
  }
  ...
  eyeL.position.set(-size * 0.12, height * 0.8, size * 0.2);
  group.add(eyeL);
  const eyeR = new THREE.Mesh(eyeGeo, eyeMat);
  eyeR.position.set(size * 0.12, height * 0.8, size * 0.2);
  group.add(eyeR);
...
function createMapBoss(mapTheme) {
  ...
  armL.position.set(-size * 0.6, height * 0.4, 0);
  armL.rotation.z = 0.3;
  group.add(armL);
  const armR = new THREE.Mesh(armGeo, armMat);
  armR.position.set(size * 0.6, height * 0.4, 0);
  armR.rotation.z = -0.3;
  group.add(armR);
  ...
  clawL.position.set(-size * 0.7, height * 0.2, 0);
  group.add(clawL);
  const clawR = new THREE.Mesh(clawGeo, clawMat);
  clawR.position.set(size * 0.7, height * 0.2, 0);
  group.add(clawR);
  ...
  legL.position.set(-size * 0.3, height * 0.15, 0);
  group.add(legL);
  const legR = new THREE.Mesh(legGeo, legMat);
  legR.position.set(size * 0.3, height * 0.15, 0);
  group.add(legR);
  ...
  eyeL2.position.set(-size * 0.12, height * 0.85, size * 0.2);
  group.add(eyeL2);
  const eyeR2 = new THREE.Mesh(eyeGeo, eyeMat);
  eyeR2.position.set(size * 0.12, height * 0.85, size * 0.2);
  group.add(eyeR2);
...
function generateBrickTexture(ctx, w, h) {
  ...
  for (let row = 0; row < h / brickH + 1; row++) {
    ...
    for (let col = -1; col < w / brickW + 2; col++) {
      ...
      for (let i = 0; i < 3; i++) {
        const bx = x + 10 + Math.random() * (brickW - 20);
        const by = y + 10 + Math.random() * (brickH - 20);
        ctx.fillRect(bx, by, 5 + Math.random() * 10, 2 + Math.random() * 4);
      }
    }
  }
...
function generateStoneTexture(ctx, w, h) {
  ...
  ctx.fillRect(0, 0, w, h);
  const stoneSize = 60;
  for (let row = 0; row < h / stoneSize + 1; row++) {
    for (let col = 0; col < w / stoneSize + 1; col++) {
      const x = col * stoneSize + ((row % 2) * stoneSize) / 2;
      ...
      for (let i = 0; i < points; i++) {
        ...
        const radius = stoneSize * 0.4 + Math.random() * 8;
        const px = x + stoneSize / 2 + Math.cos(angle) * radius;
        const py = y + stoneSize / 2 + Math.sin(angle) * radius;
...
function generateMetalTexture(ctx, w, h) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    grad.addColorStop(0.5, "rgba(80,80,80,0.3)");
...
function generateWoodTexture(ctx, w, h) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const grad = ctx.createRadialGradient(x, y, 0, x, y, radius);
    grad.addColorStop(0, "rgba(30,20,10,0.4)");
    grad.addColorStop(0.7, "rgba(40,30,20,0.3)");
...
function generateFloorTexture(ctx, w, h) {
  ...
  ctx.fillRect(0, 0, w, h);
  const tileSize = 64;
  for (let row = 0; row < h / tileSize + 1; row++) {
    for (let col = 0; col < w / tileSize + 1; col++) {
      ...
      ctx.fillRect(x + tileSize - 3, y, 2, tileSize);
    }
  }
}
...
function createWallTexture(mapType) {
  ...
  return createProceduralTexture(512, 512, generator);
}

function createFloorTexture(mapType) {
  const generator =
    mapType === "CRYPT" ? generateCryptFloor : generateFloorTexture;
  return createProceduralTexture(512, 512, generator);
}
...
function buildLevel(sector) {
  ...
  document.getElementById("mapTypeVal").textContent = mapType.name;

  isHellMode = diffConfig.hellMode || false;
  if (isHellMode) {
    isBossLevel = true;
  } else {
    isBossLevel = sector % diffConfig.bossInterval === 0 && sector > 0;
  }
  ...
  for (let n = 0; n < obstacleCount; n++) {
    ...
    while (!placed && attempts < 80) {
      ...
      if (Math.abs(i - 0) + Math.abs(j - 0) < 4) continue;
      ...
      switch (type) {
        ...
        case 2:
          ...
          mesh = new THREE.Mesh(
            new THREE.BoxGeometry(0.3, 0.5, 0.8),
            barrierMat,
          );
          mesh.castShadow = true;
          mesh.receiveShadow = true;
  ...
  // Spawn enemies based on map type
  enemies = [];
  bossEnemy = null;
  ...
  if (isHellMode) {
    ...
    for (let i = 0; i < bossCount; i++) {
      ...
      boss.userData.maxHp = getBossHealth(level) * (1 + i * 0.2);
      boss.userData.hp = boss.userData.maxHp;
      boss.userData.scareTimer = 0;
      boss.userData.height = 3.0;
      boss.userData.attackRange = 2.0 + i * 0.1;
      boss.userData.attackCooldown = 1.2 - i * 0.05;
      boss.userData.attackTimer = Math.random() * 0.5;
      boss.userData.damageMult = diffConfig.bossDamageMult * (1 + i * 0.1);
      boss.userData.bossDamage = getBossDamage() * (1 + i * 0.1);
      boss.userData.bossIndex = i;
      boss.userData.abilityCooldowns = {
        PULL: 0,
        SUMMON: 0,
        TELEPORT: 0,
        SHOCKWAVE: 0,
      };
      boss.userData.collisionRadius = 0.9;
    ...
    boss.userData.path = null;
    boss.userData.repathTimer = 0.2 + Math.random() * 0.3;
    boss.userData.speed = diffConfig.enemySpeed * 0.5;
    boss.userData.maxHp = getBossHealth(level);
    boss.userData.hp = boss.userData.maxHp;
    boss.userData.scareTimer = 0;
    boss.userData.height = 3.0;
    ...
    boss.userData.bossDamage = getBossDamage();
    boss.userData.bossIndex = 0;
    boss.userData.abilityCooldowns = {
      PULL: 0,
      SUMMON: 0,
      TELEPORT: 0,
  ...
  if (!savedLevelLoaded) {
    ...
    document.getElementById("sanityBar").querySelector("span").style.width =
      "100%";
    health = getMaxHealth();
  }
  ...
  document.getElementById("shieldDisplay").style.opacity =
    shieldAmount > 0 ? 1 : 0;
  ...
  if (isBossLevel || isHellMode) {
    ...
    document
      ...
      .querySelector(".val")
      .classList.add("boss-objective");
    document.getElementById("objectiveVal").textContent =
      "⚔ DEFEAT THE BOSS(ES) ⚔";
  } else {
    document
      .getElementById("objective")
      .querySelector(".val")
      .classList.remove("boss-objective");
...
function spawnMapEnemies(count, sector, mapType) {
  ...
  for (let n = 0; n < count; n++) {
    ...
    while (!placed && attempts < 80) {
      ...
      enemy.userData.isBoss = false;
      enemy.userData.type = typeDef.id;
      enemy.userData.cell = { i, j };
      enemy.userData.path = null;
      enemy.userData.repathTimer = 0.2 + Math.random() * 0.3;
      enemy.userData.speed =
        diffConfig.enemySpeed * typeDef.speedMult + (Math.random() * 0.2 - 0.1);
      enemy.userData.maxHp = maxHp;
      enemy.userData.hp = maxHp;
      enemy.userData.scareTimer = 0;
      enemy.userData.height = typeDef.height;
  ...
  SUMMON: {
    ...
    execute: function (boss, player, scene, enemies) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const summoned = createMapMonster("spitter", 0.5, 0.8, "dark");
        summoned.position.copy(pos);
        summoned.userData.isEnemy = true;
        summoned.userData.isBoss = false;
        summoned.userData.type = "summoned";
        summoned.userData.cell = {
          i: Math.floor(pos.x / CELL),
          j: Math.floor(pos.z / CELL),
        };
        summoned.userData.path = null;
        summoned.userData.repathTimer = 0.2;
        summoned.userData.speed = diffConfig.enemySpeed * 1.5;
        summoned.userData.maxHp = 20 + level * 2;
        summoned.userData.hp = summoned.userData.maxHp;
        summoned.userData.scareTimer = 0;
        summoned.userData.height = 0.8;
        summoned.userData.attackRange = 1.2;
        summoned.userData.attackCooldown = 0.8;
        summoned.userData.attackTimer = 0;
        summoned.userData.damageMult = 0.6;
        summoned.userData.collisionRadius = 0.4;
        summoned.castShadow = true;
        summoned.receiveShadow = true;
        scene.add(summoned);
    ...
    execute: function (boss, player) {
      ...
      if (dist < 5) {
        ...
        if (shieldAmount > 0) {
          ...
          document.getElementById("shieldVal").textContent =
            Math.round(shieldAmount);
          document.getElementById("shieldDisplay").style.opacity =
            shieldAmount > 0 ? 1 : 0;
        }
        health -= dmg;
  ...
  enemies.forEach((en) => {
    ...
    if (en.userData.isBoss && diffConfig.bossAbilities) {
      if (Math.random() < 0.015 && en.userData.attackTimer <= 0) {
        ...
        for (const key of abilityKeys) {
          if (en.userData.abilityCooldowns[key] <= 0) available.push(key);
        }
        if (available.length > 0) {
          const abilityKey =
          ...
          if (ability) {
            ...
            switch (abilityKey) {
              case "PULL":
                success = ability.execute(en, playerPos);
                break;
              case "SUMMON":
            ...
            if (success) {
              ...
              if (abilityKey === "SHOCKWAVE" || abilityKey === "PULL") {
                if (!lastPlayerPos) lastPlayerPos = playerPos.clone();
                if (playerPos.distanceTo(lastPlayerPos) > 1.5) {
                  dodges++;
                  stats.dodges++;
                  updateStatsUI();
                  AudioSys.dodge();
    ...
    if (distToPlayer < attackRange && en.userData.attackTimer <= 0) {
      ...
      if (en.userData.type === "spitter" && distToPlayer > 1.5) {
        const defenseBonus = getDefenseBonus();
        let dmg = Math.max(1, damage * 0.6 - defenseBonus);
        if (shieldAmount > 0) {
          const absorbed = Math.min(shieldAmount, dmg);
          shieldAmount -= absorbed;
          dmg -= absorbed;
    ...
    en.position.y = Math.sin(performance.now() * 0.004 + en.id) * bobAmount;
    en.rotation.z =
      Math.sin(performance.now() * 0.003 + en.id * 1.5) * swayAmount;
...
// ============================================================
// PLAYER UPDATE (FIXED - No automatic movement)
// ============================================================
function updatePlayer(dt) {
  ...
  // Only get input if we're not in a paused or menu state
  if (gameActive && !gamePaused) {
    if (isMobile && touchController) {
      ...
      // Keyboard input - only if keys are actually pressed
      if (keys["w"] || keys["W"]) {
        mx -= forward.x;
        mz -= forward.z;
      }
  ...
  // Clamp movement values to prevent drift
  ...
  if (currentWeapon.isMelee) {
    ...
    for (const enemy of enemies) {
      ...
      if (activePerks.includes("assassin")) {
        ...
        if (enemyForward.dot(toEnemyDir) > 0.5) {
          totalDamage *= 3;
        }
      }
    ...
    if (hits.length && hits[0].object.userData.isEnemy) {
      const enemy = hits[0].object;
      const headPos = enemy.position.clone();
      headPos.y += enemy.userData.height * 0.85;
...
function renderShop(tab = currentShopTab) {
  ...
  if (tab === "weapons") {
    ...
    WEAPONS.forEach((w) => {
      ...
      if (!weaponGroups[type]) weaponGroups[type] = [];
      weaponGroups[type].push(w);
    });

    Object.keys(weaponGroups).forEach((group) => {
    ...
    Object.keys(PERKS).forEach((perkId) => {
      ...
      div.className = "shop-item";
      div.innerHTML = `
                <div class="name">${perk.icon} ${perk.name}</div>
                <div class="price">⭐ ${perk.cost}</div>
...
function useTool(toolId) {
  ...
  switch (toolId) {
    ...
    case "sanity_boost":
      ...
      document.getElementById("sanityVal").textContent =
        Math.round(sanity) + "%";
      document.getElementById("sanityBar").querySelector("span").style.width =
        sanity + "%";
...
function checkDifficultyUnlocks() {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    if (unlockInfo.require && unlockInfo.requireLevel) {
      if (
        unlockedDifficulties.includes(unlockInfo.require) &&
        highestLevelCleared >= unlockInfo.requireLevel
      ) {
...
function updateUnlockInfo() {
  ...
  if (nextDiff) {
    ...
    info.textContent = `🔒 Complete ${unlockInfo.requireLevel} levels on ${unlockInfo.require.toUpperCase()} to unlock ${unlockInfo.label}`;
  } else {
...
class TouchController {
  ...
  setupTouchEvents() {
    ...
    document
      .getElementById("actionHeavy")
      .addEventListener("touchstart", (e) => {
        e.preventDefault();
        tryHeavyAttack();
      });
    document
      .getElementById("actionJump")
      .addEventListener("touchstart", (e) => {
        e.preventDefault();
        tryJump();
      });
    document
      .getElementById("actionReload")
      .addEventListener("touchstart", (e) => {
        e.preventDefault();
        startReload();
      });
    document
      .getElementById("actionFlashlight")
      .addEventListener("touchstart", (e) => {
        e.preventDefault();
        toggleFlashlight();
      });
    ...
    const grenadeBtn = document.getElementById("actionGrenade");
    if (grenadeBtn) {
  ...
  handleJoystick(e) {
    ...
    this.joystickHandle.style.transform = `translate(${-50 + (dx / maxDist) * 50}%, ${-50 + (dy / maxDist) * 50}%)`;
  }
...
document.getElementById("continueBtn").style.display = hasSave
  ? "block"
  : "none";
...
console.log("🔫 BUNKER-9 :: DELIRIUM SWEEP loaded!");
console.log("🗺️ 6 Unique Map Types with themed enemies!");
console.log("🔫 16+ Polygon Weapons with unique models!");
console.log("👾 Dynamic monster behaviors!");
console.log("💣 Grenade: Press G (30s cooldown, 50% horde chance!)");
console.log("⚔ Horde: 10-50 enemies spawn on 50% chance!");
console.log(
  "🎮 Controls: WASD | Mouse | LMB Attack | RMB Heavy | R Flashlight | F Flare | G Grenade | ESC Pause",
);
console.log("🛒 Visit the shop to buy weapons, upgrades, and perks!");
